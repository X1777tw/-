...
df = pd.read_csv('env0.csv') 
test_df = pd.read_csv('envTest.csv') 
...
model.add(Dense(32,input_shape=((20, 16)), kernel_initializer='normal', activation='relu'))


# output shape: (1, 1)
# model.add(Activation("relu")) #原本沒有
model.add(Dropout(0.2))
model.add(Dense(8,activation='softmax'))#最後一層的的激勵函數使用softmax
...
history = model.fit(X_train, Y_train, epochs=1000, batch_size=128, validation_data=(X_val, Y_val), callbacks = callback) #原本epochs=1000 batch_size = 64
